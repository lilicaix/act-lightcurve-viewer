...
def plot_iqu_maps(coadded_thumbnails, sources, bands=['f090', 'f150', 'f220']):
    for source in sources:
        for band in bands:
            ...
            clean_title = base_filename.replace(".png", "").replace("_", " ").replace("RA", "RA: ").replace("Dec", ", Dec: ")
...
def plot_time_evolution(coadded_thumbnails, sources, mean_observation_times, coadd_days, bands=['f090', 'f150', 'f220']):
    flux_ref = 150 
    for source in sources:
        for band in bands:
            print(source, band)
            ...
            clean_title = filename.replace(".png", "").replace("_", " ").replace("RA", "RA: ").replace("Dec", ", Dec: ")
            ...
            if len(mean_observation_times[source][band])<25:
                ...
                for ni in range(ntimebins):
                    ...
                    for i in range(len(coadded_thumbnails)):
                        if coadded_thumbnails[i]['source_id']!=source or coadded_thumbnails[i]['freq']!=band or int(np.mean(coadded_thumbnails[i]['coadded_observation_times']))!=mean_observation_times[source][band][ni]:
                            continue
                ...
                for ni in range(ntimebins):
                    ...
                    for i in range(len(coadded_thumbnails)):
                        if coadded_thumbnails[i]['source_id']!=source or coadded_thumbnails[i]['freq']!=band or int(np.mean(coadded_thumbnails[i]['coadded_observation_times']))!=mean_observation_times[source][band][ni]:
                            continue
                        plt.imshow(coadded_thumbnails[i]['rho'][0]*coadded_thumbnails[i]['kappa'][0]**(-0.5),
                                   vmin=-3,
                                   vmax=3
                                  )
                    # Convert raw Unix timestamp to formatted UTC datetime string for title
...
def plot_lightcurves_per_source(time, source_names, bands, flux, dflux):
    ...
    for source in np.unique(source_names):
        ...
        clean_title = base_filename.replace(".png", "").replace("_", " ").replace("RA", "RA: ").replace("Dec", ", Dec: ").replace(" all bands", "")
...
def calculate_and_plot_noise_cuts(bands, dflux):
    ...
    plt.savefig(os.path.join("aggregate_summaries", "noise_cuts_histogram.png"), bbox_inches='tight')
    plt.close()
    return noise_cuts
...
def plot_all_sources_cut_lightcurves(time, flux, dflux, bands, noise_cuts):
    ...
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    plt.gcf().autofmt_xdate()
    plt.savefig(os.path.join("aggregate_summaries", "all_sources_cut_lightcurves.png"), bbox_inches='tight')
    plt.close()
...
def plot_flux_histogram_cut(flux, dflux, bands, noise_cuts):
    ...
    for b in sorted(np.unique(bands)):
        ...
        plt.hist(flux[inds],
                 bins=21,
                 label='%s : %.2f+-%.2f'%(b,np.average(flux[inds],weights=1./dflux[inds]**2),np.nanstd(flux[inds])/np.sqrt(np.sum(inds))),
                 alpha=0.5
                )
...
def plot_coadded_aggregate(time, flux, dflux, bands, ndays=30):
    ...
    plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
    plt.gcf().autofmt_xdate()
    plt.savefig(os.path.join("aggregate_summaries", f"coadded_{ndays}day_lightcurves.png"), bbox_inches='tight')
    plt.close()
...
def plot_cut_lightcurves_per_band(time, flux, dflux, bands, noise_cuts):
    ...
    for b in np.unique(bands):
        ...
        plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%Y-%m-%d'))
        plt.gcf().autofmt_xdate()
        save_path = os.path.join("lightcurves_per_band", f"all_sources_cut_{safe_band}_lightcurve.png")
        plt.savefig(save_path, bbox_inches='tight')
        plt.close()
