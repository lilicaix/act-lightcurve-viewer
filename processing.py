...
def coadd_thumbnails(thumbnails,
                     coadd_arrays=['all'],
                     time_delta = None,
                     time_range = [],
                     time_bins = [],
    ...
    for source in thumbnails:
        ...
        for thumb in thumbnails[source]:
            ...
            if thumb_arr not in coadded_thumbs[source][thumb_time_bin][thumb_freq]:
                ...
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['rho'] = thumb.pop('rho')
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['kappa'] = thumb.pop('kappa')
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['coadded_map_ids'] = [thumb.pop('Id')]
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['coadded_observation_times'] = [thumb.pop('t')]
            else:
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['rho'] += thumb.pop('rho')
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['kappa'] += thumb.pop('kappa')
                coadded_thumbs[source][thumb_time_bin][thumb_freq][thumb_arr]['coadded_map_ids'] += [thumb.pop('Id')]
...
def coadd(time,
    ...
    for i in range(len(time_bins)):
        if np.sum(inds==i)<minobs:
            ...
            continue
        #indx = inds
        avg_flux = np.sum(flux[inds==i]*(1./dflux[inds==i])**2)/np.sum((1./dflux[inds==i])**2)
